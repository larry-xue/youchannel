...
export const createConversationFn = createServerFn({ method: "POST" })
  .inputValidator((data) => z.object({ channelId: z.string().optional(), title: z.string().optional(), selections: z.array(z.object({ videoId: z.string(), analysisId: z.string().optional() })) }).parse(data))
  .handler(
    async ({
      data,
    }) => {
...
export const sendConversationMessageFn = createServerFn({ method: "POST" })
  .inputValidator((data) => z.object({ conversationId: z.string(), content: z.string() }).parse(data))
  .handler(
  async ({ data }) => {
    const { supabase, user } = await getSupabaseAndUser();
